...
def print_best_path(pl_module):
    if pl_module.global_rank == 0:
        ...
        logger.warning(
            f"best_model_path(score:{best_model_score}): {best_model_path}")
        logger.warning(
            f"last_model_path(score:{current_score}): {last_model_path}")


def configure_optimizers(pl_module):
    params = list(pl_module.model.parameters())
    if pl_module.hparams.optim.name == 'adamw':
        opt = torch.optim.AdamW(params, lr=pl_module.hparams.optim.params.lr,
                                betas=(pl_module.hparams.optim.params.beta1,
                                       pl_module.hparams.optim.params.beta2),
                                weight_decay=pl_module.hparams.optim.params.wd)
        if pl_module.global_rank == 0:
            logger.warning('applied adamw')
    ...
    if pl_module.hparams.optim.scheduler_config is not None:
        scheduler = instantiate_from_config(
            pl_module.hparams.optim.scheduler_config)
        if pl_module.global_rank == 0:
