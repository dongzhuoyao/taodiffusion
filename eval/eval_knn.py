...
def get_embedding_extract_fn(fid_kwargs, name="simclr"):
    if name == "simclr":
        ...
        def feat_extract_fn(_imgs):
            _imgs = rearrange(
                _imgs, "b c w h ->b (c w h)").to(torch.float32).cpu()
            return _imgs
...
def get_wandbimg_by_knn(q_feats, q_imgs, imgs_gallery, nn_obj_gallery):
    ...
    for _i in range(len(query_result)):
        ...
        cur_row = np.concatenate(
            [q_imgs[_i: _i + 1], result_img_row], 0)  # [B,3,W,H]
...
def knn_papervis(
    ...
    for _i, result_idx in enumerate(query_result):
        ...
        for result_id in result_idx:
            ...
            result_img_single = draw_bbox(
                result_img_single, color=color) / 255.0
            tensor_list.append(result_img_single)

        assert len(tensor_list) >= grid_size
        tensor_list = tensor_list[:grid_size]
        query_row_concated = make_grid(
            tensor_list,
            nrow=grid_size,
            scale_each=True,
