...
from dataset.transforms.torch_utils import persistence
from dataset.transforms.torch_utils import misc
from dataset.transforms.torch_utils.ops import upfirdn2d
from dataset.transforms.torch_utils.ops import grid_sample_gradfix
from dataset.transforms.torch_utils.ops import conv2d_gradfix
...
class AugmentPipe(torch.nn.Module):
    def __init__(self,
                 xflip=0, yflip=0, rotate_any=0, xint=0, xint_max=0.125,
        ...
        self.yflip = float(yflip)  # Probability multiplier for x-flip.
        # Probability multiplier for 90 degree rotations.
        self.rotate_any = float(rotate_any)
    ...
    def forward(self, images, debug_percentile=None):
        ...
        if self.xflip > 0:
            assert self.xflip == 1.0  # 100% probability
            i = torch.rand([batch_size], device=device)
            G_inv = G_inv @ scale2d_inv(1 - 2 * i, 1)
            condition_dict['xflip'] = i

        if self.yflip > 0:
            i = torch.rand([batch_size], device=device)  # 12% probability
            i = torch.where(torch.rand(
                [batch_size], device=device) < self.yflip * self.p, i, torch.zeros_like(i))
            G_inv = G_inv @ scale2d_inv(1, 1 - 2 * i)
            condition_dict['yflip'] = i
        ...
        if self.scale > 0:
            ...
            condition_dict['scale'] = torch.log2(s) / self.scale_std
        else:
            condition_dict['scale'] = 0

        # Apply 90 degree rotations with probability (rotate90 * strength).
        if self.rotate_any > 0:
            theta = (torch.rand([batch_size], device=device)
                     * 2 - 1) * np.pi  # [0,1]-[-pi, +pi]
            theta = torch.where(torch.rand(
                [batch_size], device=device) < self.rotate_any * self.p, theta, torch.zeros_like(i))
            G_inv = G_inv @ rotate2d_inv(-theta)
            condition_dict['rotate_any0'] = torch.cos(theta) - 1
            condition_dict['rotate_any1'] = torch.sin(theta)
        ...
        if self.xint > 0:
            ...
            condition_dict['xint0'] = t[:, 0]
            condition_dict['xint1'] = t[:, 1]
        # --------------------------------------------------------
        ...
        if self.rotate > 0:
            ...
            condition_dict['rotate'] = torch.sin(theta)
        ...
        if self.rotate > 0:
            ...
            condition_dict['rotate1'] = torch.sin(theta)

        # Apply fractional translation with probability (xfrac * strength).
        if self.xfrac > 0:
            raise ValueError  # To be decided, temporarilly disable it
        ...
        if True:  # 7
            auglvl_code = torch.zeros(
                (len(images), len(condition_dict.keys())), dtype=torch.float).to(images.device)
            auglvl_code[:, 0], auglvl_code[:, 1], auglvl_code[:,
                                                              2] = condition_dict['xflip'], condition_dict['yflip'], condition_dict['scale']
            auglvl_code[:, 3], auglvl_code[:, 4], auglvl_code[:, 5], auglvl_code[:, 6] = condition_dict['xint0'], condition_dict['xint1'], \
                condition_dict['rotate_any0'], \
                condition_dict[
                'rotate_any1']
        else:
            return images, None
            auglvl_code = torch.zeros(
                (len(images), 1), dtype=torch.float).to(images.device)
            auglvl_code[:, 0], auglvl_code[:, 1], auglvl_code[:,
                                                              2] = condition_dict['xflip'], condition_dict['yflip'], condition_dict['scale']
            auglvl_code[:, 3], auglvl_code[:, 4], auglvl_code[:, 5], auglvl_code[:, 6] = condition_dict['xint0'], condition_dict['xint1'], \
                condition_dict['rotate_any0'], \
...
if __name__ == '__main__':
    if False:
        img = torch.randint(0, 255, (16, 3, 256, 256), dtype=torch.float32)
        # img = torch.rand((16, 3, 256, 256), dtype=torch.float32)
        prob = 0.12
        # aug = AugmentPipe(xflip=1.0, yflip=prob,scale=prob,rotate=prob, xint=prob)
        aug = AugmentPipe(xflip=1.0, yflip=0, scale=0,
                          xint=0, rotate_any=0,  scale_std=0.2)
        print(img.max(), img.min())
        out, auglvl_code = aug(img)
        print(out.max(), out.min(), out.shape)
    else:
        ds_root = '~/data/sg_fid_eval/grumpycat100v2_size64'
        ds_root = Path(ds_root).expanduser().resolve()
        imgs = os.listdir(ds_root)
        imgs = [os.path.join(ds_root, x) for x in imgs]
        imgs = imgs[:6]
        ...
        for img in imgs:
            img = Image.open(img)
            img = np.array(img)
            img_bs.append(torch.from_numpy(img).unsqueeze(0))

        img = torch.cat(img_bs, 0).float()

        prob = 0.0001
        aug = AugmentPipe(xflip=1.0, yflip=prob, scale=0,
                          rotate_any=prob, xint=prob, scale_std=0.2)
        #aug = AugmentPipe(xflip=1.0, yflip=0, scale=0,xint=0, rotate_any=0,  scale_std=0.2)
        out, auglvl_code = aug(img)
        plt.figure()
        for num, x in enumerate(out):
            plt.subplot(1, len(out), num+1)
            plt.axis('off')
