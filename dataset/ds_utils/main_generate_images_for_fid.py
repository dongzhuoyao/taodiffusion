...
from dataset.cifar10_torchvision import CIFAR10, CIFAR100
...
def generate_cifar(set_name='cifar100', debug=True, shuffle=True, bs=128):
    for subset, is_train in [('val', False), ('train', True)]:
        img_save_path = (
            Path(os.path.join(TMP_DIR, f"{set_name}v2_{subset}_10k"))
            .expanduser()
            .resolve()
        )
        if set_name == 'cifar100':
            ds = CIFAR100(root='~/data', train=is_train, debug=debug,
                          img_save_path=img_save_path)
        elif set_name == 'cifar10':
            ds = CIFAR10(root='~/data', train=is_train, debug=debug,
                         img_save_path=img_save_path)
        else:
...
def generate_100shot(set_name='obama100', _version='v2', debug=True, size=64, shuffle=True, bs=10):

    img_save_path = (
        Path(os.path.join(TMP_DIR, f"{set_name}{_version}_size{size}"))
        .expanduser()
        .resolve()
    )

    if set_name == 'obama100':
        data_folder = '100-shot-obama'
    elif set_name == 'panda100':
        data_folder = '100-shot-panda'
    elif set_name == "grumpycat100":
        data_folder = '100-shot-grumpy_cat'
    elif set_name == 'afcat':
        data_folder = 'AnimalFace-cat'
    elif set_name == 'afdog':
        data_folder = 'AnimalFace-dog'
    else:
        raise NotImplementedError
    root = Path(f'~/data/100_shot/{data_folder}').expanduser().resolve()
    ds = DirectoryDataset(root=root,  dataset_name=set_name, debug=debug, size=size, num_samples=None,
                          img_save_path=img_save_path)
...
def generate_ffhq(size=64, debug=True, version='v2', shuffle=True, bs=128, total_num=10_000):
    ...
    for subset, is_train in [("train", True), ("val", False)]:
        img_save_path = (
            Path(os.path.join(TMP_DIR, f"ffhq{size}{version}_{subset}_10k"))
            .expanduser()
            .resolve()
        )
        ds = FFHQ_v2(
...
if __name__ == "__main__":
    ...
    p.add_argument(
        ...
        type=str,
        default="ffhq",
        choices=["inp", "inp_original", "inp_lt", "in", "coco", 'cifar10', 'cifar100',
    ...
    if args.ds in ["inp"]:
        ...
        generate_cifar(set_name=args.ds, debug=args.debug)
    elif args.ds in ['obama100', 'panda100', 'grumpycat100', 'afcat', 'afdog']:
        generate_100shot(set_name=args.ds, debug=args.debug)
