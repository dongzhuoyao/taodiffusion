...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        # classifier-free related
        if self.condition_method == 'gridmask':
            in_channels = in_channels + 1
            self.in_channels = self.in_channels + 1
            logger.warning('add gridmask channel')
    ...
    def forward_with_cond_scale(self, x, t, cond_scale, cond=None, mask=None):
        ...
        if isinstance(cond_scale, int) and cond_scale == 1:
            epsilon_zc = self.forward(
                x=x, timesteps=t, cond_drop_prob=p0, cond=cond, mask=mask,
            )  # epsilon(z,c)
            ...
            epsilon_z = self.forward(
                x=x, timesteps=t, cond_drop_prob=p1, cond=cond, mask=mask,
            )  # epsilon(z)
    ...
    def forward(
            self, x, timesteps=None, cond_drop_prob=0.0, cond=None, mask=None, target=None, loss_fn=None):
        """
        ...
        if cond is not None:
            cond_condensed = self.cond_mlp(cond)
        ...
        h = self.out(h)
        if self.condition_method == 'gridmask' and self.training:
            mask_no = 1 - mask
            loss = loss_fn(h*mask_no, target*mask_no)
            _div = torch.sum(mask_no, (1, 2, 3), keepdim=True)
            mask_num = mask_no.size(1)*mask_no.size(2)*mask_no.size(3)
            loss = loss * mask_num / (_div + 1e-8)
            return loss
        else:
            return h
