...
from dynamic.attention import SpatialTransformer
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        dropout=0,
        # TODO, change it according to Improved DDPM
        ...
        # classifier-free related
        logger.info("*" * 100)
        self.null_cond_emb = self.mlp_cond = None
        self.cond_dim = cond_dim = 0 if cond_dim is None else cond_dim
        if cond_dim > 0:
            self.null_cond_emb = nn.Parameter(
                torch.zeros(1, cond_dim), requires_grad=False
            )

            self.mlp_cond_out = time_embed_dim // 2
            self.mlp_cond = nn.Sequential(
                nn.Linear(cond_dim, self.mlp_cond_out),
                nn.SiLU(),
                nn.Linear(self.mlp_cond_out, self.mlp_cond_out),
            )

            logger.info("has_cond!!!!!")
            logger.warning(
                f"cond_dim={cond_dim}, mlp_cond_dim_out={self.mlp_cond_out}")
            count_params(self.mlp_cond, verbose=True)
        else:
            self.mlp_cond_out = 0
            logger.warning("cond_dim=0 !!")

        self.cond_dim = cond_dim
        time_embed_dim_withcond = time_embed_dim + self.mlp_cond_out
        ...
        for level, mult in enumerate(channel_mult):
            for _ in range(num_res_blocks):
                ...
                logger.warning(f"level={level}, resblock_{_}, ds={ds}")
                if ds in attention_resolutions:
                    logger.warning(f"apply attention, ds={ds}")
                    if num_head_channels == -1:
        ...
        for level, mult in list(enumerate(channel_mult))[::-1]:
            for i in range(num_res_blocks + 1):
                ...
                logger.warning(f"up level={level}, resblock_{i}, ds={ds}")
                if ds in attention_resolutions:
                    logger.warning(f"Up, apply attention, ds={ds}")
                    if num_head_channels == -1:
                        dim_head = ch // num_heads
                    else:
                        num_heads = ch // num_head_channels
    ...
    def get_guided_score(self, z, zc, w):
        if self.condition.scale_type == "imagen":
            return (1 - w) * z + w * zc  # = z + w * (zc - z)
        elif self.condition.scale_type == "cfg":
            return (1 + w) * zc - w * z  # = zc + w * (zc - z)
        else:
            raise ValueError(self.condition.scale_type)

    def forward_with_cond_scale(
        self, x, t, cond_scale, cond,  layout=None, p0=None
    ):
        if p0 is None:
            p0 = torch.full((x.shape[0],), 0.0,
                            dtype=torch.float, device=x.device)
        p1 = torch.full((x.shape[0],), 1.0, dtype=torch.float, device=x.device)
        if is_number(cond_scale) and cond_scale == 1:
            ...
            if isinstance(epsilon_zc, tuple):
                ...
                return epsilon_zc
        elif is_number(cond_scale) and cond_scale == 0:
    ...
    def forward(
        self, x, timesteps=None, cond=None, layout=None, cond_drop_prob=0.0,
    ):
        context = None
        if isinstance(cond_drop_prob, float) or isinstance(cond_drop_prob, int):
            cond_drop_prob = torch.full(
                (x.shape[0],), cond_drop_prob, dtype=torch.float, device=x.device
            )
        ...
        if self.cond_dim > 0:
            ...
            cond_masked = self.mlp_cond(cond_masked)
            emb = torch.cat((emb, cond_masked), dim=-1)
    ...
    def get_mask(self, batch_size, cond, cond_drop_prob, device):
        assert cond.shape[1] == self.cond_dim, f"wtf? {cond.shape[1]}, {self.cond_dim}"
        cond_drop_mask = prob_mask_like(
            (batch_size,), cond_drop_prob, device=device)
        cond = torch.where(
