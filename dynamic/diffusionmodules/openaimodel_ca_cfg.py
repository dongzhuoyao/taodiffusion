...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        if self.condition_method == 'gridmask':
            ...
            self.null_cond_emb_mask2d = nn.Parameter(
    ...
    def forward(
            self, x, timesteps=None, cond_drop_prob=0.0, cond=None, mask=None):
        """
        ...
        if self.condition_method == 'gridmask':
            mask_dropouted = torch.where(
                rearrange(cond_drop_mask,
                          "b -> b 1 1 1"), self.null_cond_emb_mask2d.to(device), mask.float()
            )  # [B,cond_num]

            x = torch.cat([x, mask_dropouted], dim=1)
