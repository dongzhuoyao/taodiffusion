...
torch.set_num_threads(20)  # can drastically boost the speed in IvI cluster.
...
def run_without_decorator(cfg, run_unittest=False):
    log_dir = cfg.log_dir
    # in unittest mode, this dir doesn't be created automatically, don't know why
    ...
    if run_unittest:
        if False:
            ...
            cfg.pl.callbacks.image_logger.params.batch_frequency = 10
            cfg.data.params.batch_size = 16  # at least 16
            cfg.model.params.sampling_val = 'ddim'
            cfg.model.params.num_timesteps_val = 50  # 250
...
if __name__ == "__main__":
    # wandb.require(experiment="service")  # useful in ddp on multi-gpu training, can use it in ivi,das5, for ddp, but cannt work in das6(A6000)
    run()
